...
urlpatterns = patterns('',
    ...
    url(r'^weekly/(?P<from_date>\d\d\d\d.\d\d.\d\d)/$', 'django2wrap.views.weekly', {'run_update_before_chase': False}),
    url(r'^weekly_/$', 'django2wrap.views.weekly', {'run_update_before_chase': False}),
    url(r'^kpi/$', 'django2wrap.views.kpi'), #, {'run_update_before_chase': True}),
    url(r'^kpi_/$', 'django2wrap.views.kpi', {'run_update_before_chase': False}),
    url(r'^monthly/(?P<sfdc>\w+?)/$', 'django2wrap.views.monthly'),
    url(r'^temp_emails/(?P<sfdc>\w+?)/$', 'django2wrap.views.temp_email_lists'),
    url(r'^temp_emails_/(?P<sfdc>\w+?)/$', 'django2wrap.views.temp_email_lists', {'run_update_before_chase': False}),
